...
```json
{
  "Title": "应用名称", //升级程序展示的标题（建议）
  "InstallPath": "安装路径",//待升级应用的安装路径（建议，默认当前更新程序所在路径）
  "AutoStart": true, //升级完成后是否自动启用应用（默认true）
  "AutoClose": true, //升级完成后是否关闭更新程序（默认true）
  "ExecuteFile": "主程序文件名", //启动文件（自动启动时必选）
  "ExecuteArgs": "启动参数", //启动参数（可选）
  "AppInfo": //待升级应用信息
  {
    "types": 0,
    "code": "app_code", //应用代码
  ...
  "VerInfo": //待升级应用版本
  {
	...
	"minor": 1,//次要版本
	"patch": 1,//修订版本
	"build": 1,//构建版本（必选，升级依据）
    "phase": 5,//研发阶段：1内测，2预览，3公测，4候选，5稳定
    "forced": false,//是否强制更新
    "current": false,//是否当前版本
    "url": "升级包下载地址，ZIP格式",//升级文件路径（必选，下载地址）
	"size": 0,//升级文件大小
    "remark": "更新说明"//版本升级说明
  }
}
