...
namespace Com.Scm.Upgrade
{
    public class Upgrade
    {
        ...
        {
            Log("═══════════════════════════════════════════════");
            Log($"            Upgrade.Wpf v{MAJOR}.{MINOR}.{PATCH}.{BUILD}");
            Log("═══════════════════════════════════════════════");
            Log("");
                ...
                using (var fileStream = File.Create(destPath))
                {
                    ...
                    while ((bytesRead = stream.Read(buffer, 0, buffer.Length)) > 0)
                    {
                        ...
                        if (totalBytes > 0)
                        {
                            ...
                            ProgressChanged?.Invoke(progress, $"下载中 {progress}%");
                            Log($"   [下载] 进度：{progress}% ({FormatFileSize(totalRead)} / {FormatFileSize(totalBytes)})");
                        }
                        else
                        {
                            Log($"   [下载] 进度：{FormatFileSize(totalRead)}");
                        }
                    }
                }
                ...
                if (Directory.Exists(step.Source))
                {
                    ...
                    using (var archive = new ZipArchive(zipStream, ZipArchiveMode.Create))
                    {
                        for (int i = 0; i < files.Length; i++)
                        {
                            ...
                            ProgressChanged?.Invoke(progress, $"压缩中 {progress}%");
                            Log($"   [压缩] 进度：{i + 1}/{totalFiles} ({progress}%)");
                        }
                    }
            ...
            try
            {
                ...
                using (var archive = ZipFile.OpenRead(step.Source))
                {
                    ...
                    for (int i = 0; i < entries.Count; i++)
                    {
                        ...
                        ProgressChanged?.Invoke(progress, $"解压中 {progress}%");
                        Log($"   [解压] 进度：{i + 1}/{totalEntries} ({progress}%)");
                    }
                }
